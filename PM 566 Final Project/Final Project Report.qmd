...
-   **Race (Recode: White, Black, Other)**: Character variable representing the patient's race. The variable name was taken directly from the dataset (i.e., recoding was performed by SEER).

-   **Race Ethnicity**: Character variable detailing the specific ethnicity of the patient.

-   **Age (Recode with Single Ages and 90+)**: Character variable indicating the patient's age. The variable name was taken directly from the dataset (i.e., recoding was performed by SEER).

-   **Age (Recode with \<1 Year Olds)**: Character variable representing age ranges. The variable name was taken directly from the dataset (i.e., recoding was performed by SEER).

-   **Year of Diagnosis**: Integer indicating the year the patient was diagnosed.

-   **Site (Recode ICD-O-3/WHO 2008)**: Character variable specifying the type of leukemia diagnosed. The variable name was taken directly from the dataset (i.e., recoding was performed by SEER).

-   **Behavior Code (ICD-O-3)**: Character variable indicating the malignancy status.

-   **Year of Death (Recode)**: Character variable denoting whether the patient is alive or the year of death. The variable name was taken directly from the dataset (i.e., recoding was performed by SEER).
...
For my question of interest, I wanted to focus on the survival time for patients who had already passed away, so I filtered out observations that were categorized as "Alive at last contact" and only kept patients that have a year of death. It is important to note that this may introduce some bias in my dataset as factors associated with better survival will be less common in the data. After filtering, the dataset dimensions were 3457 observations by 14 variables.
